...
def create_close_dataframe(history: np.ndarray, abbreviation, date_list):
    """
    Args:
...
class DataGenerator(object):
    ...
    def reset(self):
        ...
        ti_obs = None
        if self.technical_indicators_flag:
            technical_indicator_data = self._technical_indicator_data[:,
...
class PortfolioEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.close_df = create_close_dataframe(history, abbreviation,
                                               date_list)  # Dataframe that holds the Close data in Different formats

        if self.start_date is not None and self.end_date is not None:
            self.subset_dates = date_list[date_list.index(self.start_date): date_list.index(self.end_date) + 1]
            self.close_df_subset = self.close_df[self.close_df.index.isin(
                self.subset_dates)]  # Dataframe that holds subset of the Close data in Different formats
    ...
    def _step(self, action):
        ...
        if self.olps:
            ...
            info['price_history'] = self.close_df[
                                    :_date]  # self.close_df[self.close_df.index.to_series().between('', _date)]
